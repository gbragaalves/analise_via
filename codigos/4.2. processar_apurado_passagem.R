...
nm <- list.files(path=local_gps_proc, full.names = T, pattern = "*_passam.csv",recursive = TRUE)
...
fwrite(sppo_hora_aferido,
       paste0(local_aferido, "/sppo_hora_aferido_passagem.csv"))
fwrite(
  sppo_hora_aferido_detalhado,
  paste0(local_aferido, "/sppo_hora_aferido_detalhado_passagem.csv")
)
